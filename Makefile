# Makefile to build simple

TARGET_STRING := simple
TARGET := $(TARGET_STRING)

# Preprocessor definitions
DEFINES := _FINALROM=1 F3DEX_GBI
...
SRC_DIRS += src src/buffers src/main src/main/audio src/ascii_font assets asm
...
CFLAGS = -G 0 -Os -mabi=32 -ffreestanding -mfix4300 $(DEF_INC_CFLAGS)
...
$(ELF): $(O_FILES) $(BUILD_DIR)/$(LD_SCRIPT)
	...
	$(V)$(LD) -L $(BUILD_DIR) -T $(BUILD_DIR)/$(LD_SCRIPT) -Map $(BUILD_DIR)/$(TARGET).map --no-check-sections -o $@ $(O_FILES) -L/usr/lib/n64 -lultra -L$(N64_LIBGCCDIR) -lgcc

# Build ROM
$(ROM): $(ELF)
	$(call print,Building ROM:,$<,$@)
	$(V)$(OBJCOPY) --pad-to=0x100000 --gap-fill=0x00 $< $@ -O binary
	$(V)makemask $@
